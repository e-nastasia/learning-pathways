...
pub fn anchor_modify() -> Result<(), String> {
    Err(String::from(
        "Can't modify the CourseAnchor entry: it can only be created or deleted".to_owned(),
    ))
}
...
pub fn catalog_modify() -> Result<(), String> {
    Err(String::from(
        "Can't modify the CourseAnchorCatalog entry".to_owned(),
    ))
}
...
pub fn catalog_delete() -> Result<(), String> {
    Err(String::from(
        "Can't delete the CourseAnchorCatalog entry".to_owned(),
    ))
}
